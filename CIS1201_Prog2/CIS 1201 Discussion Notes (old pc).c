...
void displayBit(int x){
	int a,temp; /*a is the number of bits to shift left*/
	for(a=SIZE-1;a>=0;a--){
		temp = ((x & 1<<a)> 0) ? 1 : 0;
		printf("%d",temp);
	}
...
/*
fseek(fp,1*(recSize),1)
fseek(fp,4*(recSize),0)
fseek(fp,-1*(recSize),2)
...
for(;indx < list-count - 2 ;indx++){
	list->Stud[ctr] = list->Stud[ctr - 1];
}
list->count--;
